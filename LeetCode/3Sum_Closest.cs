...
namespace LeetCode
{
    public class _3Sum_Closest
    {
        public int ThreeSumClosest(int[] nums, int target)
        {
            ...
            {
                int start = i + 1, end = nums.Length - 1, lookup = target - nums[i];

                while (start < end)
                {
                    int currentDiff = nums[start] + nums[end] - lookup;
                    ...
                    if (Math.Abs(target - (currentDiff + lookup + nums[i])) < Math.Abs(target - closest))
                        closest = currentDiff + lookup + nums[i];
                }
            }
        ...
        //public int ThreeSumClosest(int[] nums, int target,int start,int end, ref )
        //{
        //}
